...
var BoardTabsView = class extends import_obsidian3.ItemView {
    ...
    input.onchange = async (e) => {
      ...
      reader.onload = async (event) => {
        ...
        try {
          ...
          new import_obsidian3.Notice("Import complete");
          await this.reload();
        } catch (error) {
          new import_obsidian3.Notice("Failed to import file: " + error.message);
        }
      };
  ...
  async exportToJson() {
    ...
    const fileName = `Kanban-Export-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.json`;
  ...
  async importFromCsv() {
    ...
    input.onchange = async (e) => {
      ...
      reader.onload = async (event) => {
        var _a2;
        try {
          const data = (_a2 = event.target) == null ? void 0 : _a2.result;
          const workbook = readSync(data, { type: "string" });
  ...
  async exportToExcel() {
    ...
    const wsCr = utils.json_to_sheet(crData);
    utils.book_append_sheet(wb, wsTasks, "Tasks");
    utils.book_append_sheet(wb, wsCr, "Change Requests");
    const wbout = writeSync(wb, { bookType: "xlsx", type: "array" });
    const blob = new Blob([wbout], { type: "application/octet-stream" });
    const fileName = `Kanban-Export-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.xlsx`;
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = fileName;
    document.body.appendChild(link);
  ...
  async exportToCsv() {
    ...
    utils.book_append_sheet(wb, wsTasks, "Tasks");
    utils.book_append_sheet(wb, wsCr, "Change Requests");
    const wbout = writeSync(wb, { bookType: "csv", type: "array" });
    const blob = new Blob([wbout], { type: "text/csv;charset=utf-8;" });
    const fileName = `Kanban-Export-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.csv`;
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = fileName;
    document.body.appendChild(link);
  ...
  async processImportData(data, type) {
    ...
    for (const item of data) {
      ...
      if (item.hasOwnProperty("Archived") || item.hasOwnProperty("archived")) {
        ...
        if (val !== void 0) item.archived = val;
      }
      ...
      if (existingFile) {
        ...
        const frontmatter = Object.assign({}, item);
    ...
    for (const t of this.getFilteredTasks()) {
      ...
      for (const key of this.settings.gridConfig.visibleColumns) {
          ...
          try {
            ...
            if (inFrontmatter || fieldDef.type !== "freetext") {
              ...
              if (fieldDef.type === "tags" || fieldDef.type === "people") {
                if (typeof newVal === "string") payload = newVal.split(",").map((s) => s.trim()).filter(Boolean);
              }
        ...
        if (Boolean(t.frontmatter["archived"])) {
          ...
          displayEl.style.display = "none";
          const tagsArr = Array.isArray(t.frontmatter[key]) ? t.frontmatter[key] : t.frontmatter[key] ? String(t.frontmatter[key]).split(",").map((s) => s.trim()).filter(Boolean) : [];
          const preview = td.createDiv({ cls: "kb-tags-preview" });
          if (tagsArr.length > 0) {
            const first = preview.createDiv({ cls: "kb-tag kb-tag-large" });
            first.setText(tagsArr[0]);
            if (tagsArr.length > 1) {
              const more = preview.createDiv({ cls: "kb-tag kb-tag-more" });
              more.setText(`+${tagsArr.length - 1}`);
            }
          } else {
          ...
          preview.onclick = (e) => {
            ...
            class TagEditorModal extends import_obsidian3.Modal {
              ...
              onOpen() {
                ...
                const input = this.containerElInner.createEl("input");
                ...
                getAllExistingTags(outer.app, outer.settings).then((tags) => {
                  this.allTags = tags;
                  renderSuggestions();
                }).catch(() => {
                });
                const renderSuggestions = (q) => {
                  ...
                  let candidates = this.allTags.filter((t2) => !this.selected.includes(t2));
                  if (ql) candidates = candidates.filter((t2) => t2.toLowerCase().includes(ql));
                  if (ql && !this.allTags.map((a) => a.toLowerCase()).includes(ql)) {
                    const addOpt = sugg.createDiv({ cls: "kb-tag-suggestion" });
                    addOpt.setText(`Add "${ql}"`);
                    addOpt.onclick = () => {
                      this.selected.push(ql);
                      renderChips();
                      input.value = "";
                      renderSuggestions();
                      input.focus();
      ...
      body.ondrop = async (e) => {
        ...
        try {
          ...
          if (!draggedTask) {
            ...
            draggedTask = { file, filePath: payload.path, fileName: file.name.replace(/\.md$/, ""), frontmatter: (_k = cache == null ? void 0 : cache.frontmatter) != null ? _k : {}, subtasks: [] };
          }
          if (draggedTask) {
